...
# Rendering
# ══════════════════════════════════════════════════════════════════════
def render_gaussians(means, scales, quats, features_dc, opacities, camera, device=DEVICE, return_depth=False):
    """Render Gaussians using gsplat from a SceneCamera.

    When return_depth=True, uses render_mode='RGB+ED' and also returns a float32
    (H, W) depth map in OpenCV camera-space Z (positive = in front of camera).
    Returns (rgb, alpha) normally, or (rgb, alpha, depth) when return_depth=True.
    """
    ...
    if return_depth:
        # Pass only RGB background — gsplat appends its own 0 for the depth channel
        # internally. Passing 4 channels causes a shape mismatch ([1,5] vs [1,4]).
        ...
        out = renders[0].cpu().numpy()          # (H, W, 4)
        rgb = np.clip(out[..., :3], 0, 1)
        depth = out[..., 3].astype(np.float32)  # (H, W) camera-space Z
        return rgb, alphas[0].cpu().numpy(), depth
...
def render_depth_map(means, scales, quats, features_dc, opacities, camera, device=DEVICE):
    """Return only the (H, W) float32 depth map (camera-space Z, positive forward)."""
    _, _, depth = render_gaussians(
        means, scales, quats, features_dc, opacities, camera,
        device=device, return_depth=True,
    )
    return depth


def unproject_depth_to_world(u, v, depth_map, camera, window=7):
    """Unproject pixel (u, v) to a 3D world-space point via the depth map.

    Samples the median of valid (>0) depth values in a (window × window)
    neighbourhood to suppress noise.  Returns a (3,) float64 world-space point,
    or None if no valid depth was found in the window.

    Args:
        u, v      – pixel column / row (floats)
        depth_map – (H, W) float32 array, camera-space Z positive forward
        camera    – SceneCamera (provides w2c, fx, fy, cx, cy)
        window    – sampling half-window in pixels (default 7 → 15×15 patch)
    """
    h, w = depth_map.shape
    ui, vi = int(round(u)), int(round(v))

    r0, r1 = max(0, vi - window // 2), min(h, vi + window // 2 + 1)
    c0, c1 = max(0, ui - window // 2), min(w, ui + window // 2 + 1)
    ...
    # Unproject to OpenCV camera space (Y-down, Z-forward)
    Xc = (u - camera.cx) * d / camera.fx
    Yc = (v - camera.cy) * d / camera.fy
    Zc = d

    # camera.w2c is the OpenCV world→camera matrix; its inverse is camera→world.
    c2w_cv = np.linalg.inv(camera.w2c)
    P_world = c2w_cv @ np.array([Xc, Yc, Zc, 1.0], dtype=np.float64)
    return P_world[:3]


def save_depth_map_png(depth_map, output_path):
    """Save depth map as a jet-colourised PNG and a raw .npy alongside it.

    Files written:
      <output_path>         – jet-colourised 8-bit PNG for visual inspection
      <output_path>.raw.npy – raw float32 array for downstream use
    """
    np.save(output_path + ".raw.npy", depth_map.astype(np.float32))

    valid = depth_map[depth_map > 0]
    if valid.size == 0:
        Image.fromarray(np.zeros(depth_map.shape, dtype=np.uint8)).save(output_path)
        return output_path

    d_min, d_max = float(valid.min()), float(valid.max())
    span = d_max - d_min if d_max > d_min else 1.0
    normed = np.clip((depth_map - d_min) / span, 0, 1)

    cmap = plt.get_cmap("jet")
    colored = (cmap(normed)[:, :, :3] * 255).astype(np.uint8)
    colored[depth_map <= 0] = 0   # black for pixels with no geometry
...
def select_object_gaussians_depth_aware(
    ...
    the same camera ray at a different depth — e.g. a far wall behind a
    bench. A single camera ray through the bbox can pierce both the bench
    surface and the wall far behind it; both gaussians project to the same
    pixel, so a 2D-only test cannot tell them apart. This causes the
    ...
    This adds two depth-aware passes on top of the 2D footprint test:
      ...
      1. Occlusion consistency — a candidate's own camera-space depth must
         ...
         (within `depth_tol_abs`). The rendered depth map is alpha-composited
    ...
    this alone removes most of the empty bbox margin that often grazes
    background surfaces.

    Returns:
        indices — int array into `means` for the selected gaussians.
        info    — dict of diagnostic counts/stats for logging.
    """
    u, v, z, valid = cam.project(means)
    x1, y1, x2, y2 = bbox
    in_bbox = (
        valid
        & (u >= x1) & (u <= x2)
    ...
    if mask is not None and mask.any():
        h, w = mask.shape
        ui_all = np.clip(np.round(u).astype(int), 0, w - 1)
        vi_all = np.clip(np.round(v).astype(int), 0, h - 1)
        in_mask = mask[vi_all, ui_all]
        in_bbox = in_bbox & in_mask

    cand = np.where(in_bbox)[0]
    info = {"raw_candidate_count": int(cand.size)}
    if cand.size == 0:
        return cand, info

    h, w = depth_map.shape
    ui = np.clip(np.round(u[cand]).astype(int), 0, w - 1)
    vi = np.clip(np.round(v[cand]).astype(int), 0, h - 1)
    surf_depth = depth_map[vi, ui]
    has_surf = surf_depth > 0.01

    # Pass 1: occlusion consistency.
    occlusion_ok = np.ones(cand.size, dtype=bool)
    occlusion_ok[has_surf] = np.abs(z[cand][has_surf] - surf_depth[has_surf]) < depth_tol_abs
    stage1 = cand[occlusion_ok]
    if stage1.size == 0:
...
def add_object_to_scene(
    ...
    # Guard against a resolution mismatch between the diffusion-edited image
    # (e.g. Gemini silently ignoring the requested output size) and the
    # camera's actual render_width/render_height. Everything downstream
    # (OWLv2 bbox, mask, depth-aware gaussian selection, projection) assumes
    # pixel coordinates live in the camera's pixel space, so if the input
    # image is a different resolution every coordinate is systematically
    # off (e.g. detections landing too far left/up). Resize to match before
    # any detection happens.
    if os.path.exists(camera_state_path):
        try:
            _cam_state_probe = torch.load(camera_state_path, map_location="cpu", weights_only=False)
            expected_w = int(_cam_state_probe.get("render_width", 0))
            expected_h = int(_cam_state_probe.get("render_height", 0))
            if expected_w and expected_h:
                with Image.open(object_image_path) as _probe_img:
                    actual_w, actual_h = _probe_img.size
                if (actual_w, actual_h) != (expected_w, expected_h):
                    print(
                        f"Warning: {object_image_path} is {actual_w}x{actual_h} but camera expects "
                        f"{expected_w}x{expected_h}. Resizing to match camera pixel space before detection."
                    )
                    resized_path = os.path.join(
                        SESSION_DIR, f"_resized_{os.path.basename(object_image_path)}"
                    )
                    with Image.open(object_image_path) as _src_img:
                        _src_img.convert("RGB").resize((expected_w, expected_h), Image.LANCZOS).save(resized_path)
                    object_image_path = resized_path
        except Exception as e:
            print(f"Warning: could not verify/resize {object_image_path} against camera resolution: {e}")
    ...
    # Render the scene depth map up front — needed both for depth-aware
    # gaussian selection below and for placement later. One forward pass,
    # no extra model required.
    print("Rendering depth map from selected camera for object detection/placement...")
    t_depth_start = time.time()
    depth_map = render_depth_map(means, scales, quats, features_dc, opacities_raw, cam)
    timings['Depth map render'] = time.time() - t_depth_start

    depth_png_path = os.path.join(SESSION_DIR, "placement_depth_map.png")
    save_depth_map_png(depth_map, depth_png_path)
    print(f"Saved depth map to {depth_png_path} (raw: {depth_png_path}.raw.npy)")

    # Load the tight per-pixel object mask (Gemini cutout matched into bbox)
    # if available — it's a much better 2D footprint than the raw bbox and
    # is reused below both for gaussian selection and for surface placement.
    mask_path_for_placement = os.path.join(SESSION_DIR, "added_object_mask.png")
    placement_mask = None
    if os.path.exists(mask_path_for_placement):
        placement_mask = np.array(Image.open(mask_path_for_placement).convert("L")) > 127

    # Depth-aware gaussian selection — see select_object_gaussians_depth_aware
    # docstring for why a plain 2D-bbox test is insufficient (it conflates
    # the object with unrelated background geometry along the same ray,
    # e.g. a wall far behind a bench).
    object_indices, sel_info = select_object_gaussians_depth_aware(
        means, opacities, cam, depth_map, (x1, y1, x2, y2), mask=placement_mask,
    )
...
if __name__ == "__main__":
    ...
    # Use the camera's actual render resolution (the interactive viewer window
    # size, saved in camera_state.pt) rather than the RENDER_W/RENDER_H
    # defaults — they can differ, and downstream gaussian projection assumes
    # pixel coordinates match the camera's resolution exactly.
    _camera_state_for_size = torch.load(CAMERA_STATE_PATH, map_location="cpu", weights_only=False)
    gen_width = int(_camera_state_for_size.get("render_width", RENDER_W))
    gen_height = int(_camera_state_for_size.get("render_height", RENDER_H))
