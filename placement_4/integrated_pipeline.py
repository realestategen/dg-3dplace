...
def add_object_to_scene(
    ...
    # ── Depth-map placement ──────────────────────────────────────────
    # Render a metric depth map from the selected camera (one forward pass,
    # no extra model required).  Sample at the bottom-centre of the detected
    # bbox — that pixel corresponds to where the object's base meets the
    # surface — and unproject to world space to get an exact support point.
    print("Rendering depth map from selected camera for placement...")
    ...
    # Save depth map to session folder for inspection
