...
def add_object_to_scene(object_image_path, object_obj_path, cam_idx=None):
    ...
    # --- Optimized 3D placement using pose computation and depth map ---
    ...
    # --- Load or compute depth map for the selected camera view ---
    # For demonstration, assume a depth map is available as a numpy array: depth_map (H, W)
    # You should replace this with actual depth map loading if available
    depth_map_path = os.path.join(SESSION_DIR, f"depth_cam_{cam_idx}.npy")
    if os.path.exists(depth_map_path):
        depth_map = np.load(depth_map_path)
    else:
        # Placeholder: create a fake depth map (all 1.0)
        depth_map = np.ones((RENDER_H, RENDER_W), dtype=np.float32)

    # --- Extract 2D-3D correspondences from bbox and depth ---
    # Use bbox corners and center
    ...
    object_points_3d = []
    valid_points_2d = []
    for pt in bbox_points_2d:
        u, v = int(round(pt[0])), int(round(pt[1]))
        if 0 <= u < RENDER_W and 0 <= v < RENDER_H:
            z = depth_map[v, u]
            if z > 0:
    ...
    if len(object_points_3d) >= 4:
        import cv2
        camera_matrix = cam.get_K()
        dist_coeffs = np.zeros((4, 1))
        pose_success, rvec, tvec = cv2.solvePnP(
            object_points_3d, valid_points_2d, camera_matrix, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE
        )
    if pose_success:
        print(f"Pose estimated: rvec={rvec.ravel()}, tvec={tvec.ravel()}")
    else:
        print("Pose estimation failed or insufficient correspondences. Using bbox region as fallback.")

    # --- Identify supporting surface (min depth in bbox) ---
    surface_depths = []
    for pt in bbox_points_2d:
        u, v = int(round(pt[0])), int(round(pt[1]))
        if 0 <= u < RENDER_W and 0 <= v < RENDER_H:
            z = depth_map[v, u]
            if z > 0:
                surface_depths.append(z)
    if surface_depths:
        surface_z = min(surface_depths)
    else:
        surface_z = 1.0

    # --- Compute 3D region above surface for vase placement ---
    # Assume vase height (in meters or scene units)
    vase_height = 0.25
    # Compute 3D base center from bbox center and surface_z
    u_c, v_c = int(round((x1 + x2) / 2)), int(round((y1 + y2) / 2))
    x_c = (u_c - cam.cx) * surface_z / cam.fx
    y_c = (v_c - cam.cy) * surface_z / cam.fy
    base_center_3d = np.array([x_c, y_c, surface_z])
    # The empty space is the region above base_center_3d up to vase_height
    vase_top_3d = base_center_3d + np.array([0, 0, vase_height])

    # --- Select Gaussians in the empty space (not just surface) ---
    # Transform all means to camera coordinates
    means_h = np.column_stack([means, np.ones(means.shape[0])])
    cam_pts = (cam.w2c @ means_h.T).T[:, :3]
    # Find Gaussians within a cylinder above the base center
    radius = max((x2 - x1), (y2 - y1)) * surface_z / cam.fx / 2.0
    z_min = base_center_3d[2]
    z_max = vase_top_3d[2]
    d_xy = np.linalg.norm(cam_pts[:, :2] - base_center_3d[:2], axis=1)
    in_cylinder = (
        (d_xy <= radius)
        & (cam_pts[:, 2] >= z_min)
        & (cam_pts[:, 2] <= z_max)
    ...
    object_indices = np.where(in_cylinder)[0]
    t_unproj_end = time.time()
    timings['Unprojection & 3D detection'] = t_unproj_end - t_unproj_start
    t_highlight_start = time.time()
    print(f"Gaussians in {OBJECT_CLASSNAME} empty space: {len(object_indices):,} / {len(means):,}")
